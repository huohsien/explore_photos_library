-- ============================================================================
-- 14 Favorite Navigator -- Backup Photos Library (Read-Only) -- v3
-- ============================================================================
--
...
--
-- v3 contains the 14 Favorite differences remaining after the
-- 2026-06-26 live Backup + live Current metadata refresh and report.
-- ============================================================================
...
on run
	...
	set chosenAction to choose from list actionChoices with prompt promptText with title "14 Favorite Navigator - Backup" default items {"Spotlight current record"} OK button name "Continue" cancel button name "Quit"
	...
	if selectedAction is "Reset progress to record 1" then
		...
		display dialog "Progress reset. Record 1 is ready." buttons {"OK"} default button "OK" with title "14 Favorite Navigator"
		return
	end if
	
	if selectedAction is "Open a specific record" then
		set recordChoices to my recordChoiceList(reviewTargets)
		set selectedRecordChoice to choose from list recordChoices with prompt "Choose one of the remaining Favorite differences to spotlight in the currently open Backup Photos Library." with title "Choose Favorite Difference" OK button name "Spotlight" cancel button name "Cancel"
	...
	if selectedAction is "Spotlight next record" then
		if currentIndex is greater than or equal to totalCount then
			...
			display dialog "All remaining records are already at the final position." & return & return & "Use Open a specific record to revisit any item, or Reset progress to record 1 to start again." buttons {"OK"} default button "OK" with icon note with title "14 Favorite Navigator"
			return
		end if
...
on spotlightTargetAtIndex(targetIndex, totalCount, reviewTargets)
	...
	display notification "Record " & targetIndex & " of " & totalCount & " highlighted in Photos." with title "14 Favorite Navigator" subtitle targetFilename
...
on progressFilePath()
	return my navigatorStateDirectory() & "/progress_14.txt"
