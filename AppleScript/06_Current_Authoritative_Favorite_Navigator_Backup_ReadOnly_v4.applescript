-- ============================================================================
-- 6 Current-Authoritative Favorite Navigator -- Backup Photos Library (Read-Only) -- v4
-- ============================================================================
--
...
--
-- v4 contains only the six remaining rows where Current Favorite is True
-- and Backup Favorite is False after the 2026-06-27 live refresh/report.
-- Current is authoritative for this pass: manually set Favorite to True
...
on run
	...
	set promptText to "Open the Backup Photos Library before using this navigator." & return & return & "Current is authoritative for these six records." & return & "After each item is spotlighted, manually set Favorite to True in Backup." & return & return & "Current review position: " & currentIndex & " of " & totalCount & return & targetSummary & return & return & "This script never changes Photos metadata."
	set chosenAction to choose from list actionChoices with prompt promptText with title "6 Current-Authoritative Favorite Navigator - Backup" default items {"Spotlight current record"} OK button name "Continue" cancel button name "Quit"
	...
	if selectedAction is "Reset progress to record 1" then
		...
		display dialog "Progress reset. Record 1 is ready." buttons {"OK"} default button "OK" with title "6 Current-Authoritative Favorite Navigator"
		return
	end if
	
	if selectedAction is "Open a specific record" then
		set recordChoices to my recordChoiceList(reviewTargets)
		set selectedRecordChoice to choose from list recordChoices with prompt "Choose one of the remaining Favorite differences to spotlight in the currently open Backup Photos Library." with title "Choose Current-Authoritative Favorite Target" OK button name "Spotlight" cancel button name "Cancel"
	...
	if selectedAction is "Spotlight next record" then
		if currentIndex is greater than or equal to totalCount then
			...
			display dialog "All remaining records are already at the final position." & return & return & "Use Open a specific record to revisit any item, or Reset progress to record 1 to start again." buttons {"OK"} default button "OK" with icon note with title "6 Current-Authoritative Favorite Navigator"
			return
		end if
...
on makeReviewTargets()
	...
	--
	-- Current is authoritative for this pass. All six rows are:
	--   Backup False, Current True.
	-- After spotlighting a row in the open Backup Photos Library, manually
	-- set Favorite to True for that exact item.
	set targets to {}
...
on spotlightTargetAtIndex(targetIndex, totalCount, reviewTargets)
	...
	display notification "Record " & targetIndex & " of " & totalCount & " highlighted in Photos." with title "6 Current-Authoritative Favorite Navigator" subtitle targetFilename
...
on targetSummaryForIndex(targetIndex, reviewTargets)
	...
	return "Original Filename: " & targetFilename & return & "Date: " & my formattedDate(targetRecord) & return & "Backup Favorite: " & my booleanText(backupFavorite) & return & "Current Favorite: " & my booleanText(currentFavorite) & return & "Manual action in Backup: set Favorite to True"
...
on recordChoiceList(reviewTargets)
	...
	repeat with targetIndex from 1 to totalCount
		...
		set backupFavorite to item 8 of targetRecord
		set currentFavorite to item 9 of targetRecord
		set choiceText to targetIndex & ". " & targetFilename & " | " & my formattedDate(targetRecord) & " | Backup " & my booleanText(backupFavorite) & " | Current " & my booleanText(currentFavorite) & " | Set Backup Favorite to True"
...
on progressFilePath()
	return my navigatorStateDirectory() & "/progress_6_current_authoritative_backup.txt"
